...
/**
 * Locks the mutation core of {@link LionKingForgeEvents}' two terrain-mutating handlers:
 *
 * <ul>
 *   <li>{@link LionKingForgeEvents#tryTillSand} — hoe-on-sand → tilled sand (RightClickBlock event).</li>
 *   <li>{@link LionKingForgeEvents#convertSandToOutsand} — lightning landing in the Outlands
...
public class ForgeEventsGameTests {
    ...
    public void tryTillSandConvertsSandWithAirAbove(GameTestHelper helper) {
        ...
        boolean tilled = LionKingForgeEvents.tryTillSand(helper.getLevel(), helper.absolutePos(sand));

        if (!tilled) {
            helper.fail("tryTillSand returned false on plain sand with air above");
            return;
        }
        if (!helper.getBlockState(sand).is(LionKingBlocks.TILLED_SAND.get())) {
    ...
    public void tryTillSandRejectsNonSand(GameTestHelper helper) {
        ...
        boolean tilled = LionKingForgeEvents.tryTillSand(helper.getLevel(), helper.absolutePos(stone));
    ...
    public void convertSandToOutsandReplacesSandInRadius(GameTestHelper helper) {
        ...
        LionKingForgeEvents.convertSandToOutsand(helper.getLevel(), helper.absolutePos(center));

        int outsandCount = 0;
        for (int dx = -1; dx <= 1; dx++) {
            for (int dz = -1; dz <= 1; dz++) {
                if (helper.getBlockState(center.offset(dx, 0, dz)).is(LionKingBlocks.OUTSAND.get())) {
                    outsandCount++;
                }
            }
        }
