...
import io.github.ron1196.thelionking.block.LKPortalBlock;
...
public class LKTeleporter implements ITeleporter {

    private final boolean isOutlands;

    public LKTeleporter(boolean isOutlands) {
    ...
    private BlockPos findOrCreatePortal(Entity entity, ServerLevel destWorld) {
        Block portalBlock = isOutlands ? LKBlocks.OUTLANDS_PORTAL.get() : LKBlocks.PRIDE_LANDS_PORTAL.get();
        Block frameBlock = isOutlands ? LKBlocks.OUTLANDS_PORTAL_FRAME.get() : LKBlocks.PRIDE_PORTAL_FRAME.get();
    ...
    private BlockPos findExistingPortal(ServerLevel level, BlockPos center, Block portalBlock) {
        ...
        for (int x = -range; x <= range; x++) {
            for (int z = -range; z <= range; z++) {
                ...
                if (!level.hasChunkAt(new BlockPos(checkX, 0, checkZ))) continue;
    ...
    private BlockPos createPortal(ServerLevel level, BlockPos pos, Block frameBlock, Block portalBlock) {
        ...
        if (portal.hasProperty(LKPortalBlock.AXIS)) {
            portal = portal.setValue(LKPortalBlock.AXIS, Direction.Axis.X);
        }
