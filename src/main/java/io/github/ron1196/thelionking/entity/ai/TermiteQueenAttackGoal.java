...
/**
 * Boss AI goal for the Termite Queen. - Targets nearest player - Melee attacks when close (within 3
 * blocks) - Spawns termite minions when far (every 5 seconds, up to 8 nearby)
 */
public class TermiteQueenAttackGoal extends Goal {
  ...
  private void spawnTermites() {
    ...
    List<TermiteEntity> nearby =
        queen.level().getEntitiesOfClass(TermiteEntity.class, queen.getBoundingBox().inflate(16.0));
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (termite != null) {
        ...
        termite.moveTo(
            offsetX, queen.getY(), offsetZ, queen.getRandom().nextFloat() * 360.0F, 0.0F);
