...
public class SpearEntity extends AbstractArrow implements net.minecraft.world.entity.projectile.ItemSupplier {
    ...
    protected void onHitEntity(@NotNull EntityHitResult result) {
        ...
        if (!this.level().isClientSide && poisoned && result.getEntity() instanceof LivingEntity target) {
    ...
    protected @NotNull ItemStack getPickupItem() {
        return isPoisoned()
                ? new ItemStack(LionKingItems.POISONED_SPEAR.get())
                : new ItemStack(LionKingItems.GEMSBOK_SPEAR.get());
    }
}
