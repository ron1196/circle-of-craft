...
public enum GrindingBowlProvider implements IBlockComponentProvider {
    ...
    public static final ResourceLocation UID = new ResourceLocation(TheLionKingMod.MOD_ID, "grinding_bowl");
    ...
    public void appendTooltip(
        ...
        if (input.isEmpty()) {
            tooltip.add(Component.translatable("jade.thelionking.grinding_bowl.empty")
                    .withStyle(ChatFormatting.GRAY));
            return;
        }
        tooltip.add(Component.translatable("jade.thelionking.grinding_bowl.input", input.getHoverName()));
        ...
        recipe.ifPresent(r -> tooltip.add(Component.translatable(
                "jade.thelionking.grinding_bowl.output", r.getResult().getHoverName())));

        if (bowl.isGrinding()) {
            int pct = Math.round(bowl.getGrindProgress() * 100);
            tooltip.add(Component.translatable("jade.thelionking.grinding_bowl.progress", pct));
        }
    }
