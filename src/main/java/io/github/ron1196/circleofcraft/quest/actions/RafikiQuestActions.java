...
import io.github.ron1196.circleofcraft.block.ZiraMoundGateBlock;
...
public final class RafikiQuestActions {
    ...
    private static void ensureOutlandsPortalOpen(ServerLevel level) {
        Block gateBlock = ModBlocks.ZIRA_MOUND_GATE.get();
        PortalBlock portalBlock = (PortalBlock) ModBlocks.OUTLANDS_PORTAL.get();
        ...
        for (Player player : level.players()) {
            ...
            if (findNearbyBlock(level, center, ModBlocks.OUTLANDS_PORTAL.get(), GATE_SEARCH_RADIUS) == null) {
    ...
    private static BlockPos findRafikiTreeNear(ServerLevel level, BlockPos center) {
        return findNearbyBlock(level, center, ModBlocks.RAFIKI_WOOD.get(), RAFIKI_TREE_SEARCH_RADIUS);
    }
