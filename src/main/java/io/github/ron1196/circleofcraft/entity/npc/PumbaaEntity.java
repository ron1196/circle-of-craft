...
import io.github.ron1196.circleofcraft.entity.ai.PumbaaFollowTimonGoal;
...
public class PumbaaEntity extends PathfinderMob {
    ...
    private void spawnPumbaaBox() {
        ...
        // Spawn the box item
        var boxStack = new ItemStack(ModBlocks.PUMBAA_BOX.get());
    ...
    private static boolean playerHasPumbaaBox(@NotNull Player player) {
        for (ItemStack stack : player.getInventory().items) {
            if (stack.is(ModBlocks.PUMBAA_BOX.get().asItem())) {
                return true;
            }
        }
        return false;
