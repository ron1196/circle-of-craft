...
public class TimonEntity extends PathfinderMob {
    ...
    protected @NotNull InteractionResult mobInteract(@NotNull Player player, @NotNull InteractionHand hand) {
        ...
        if (held.is(ModItems.BUG.get()) && held.getCount() >= 5) {
            held.shrink(5);
            int reward = random.nextInt(4);
            switch (reward) {
                case 0 -> player.addItem(new ItemStack(ModItems.PUMBAA_BOMB.get(), 3));
                case 1 -> player.addItem(new ItemStack(ModItems.CRYSTAL.get(), 1));
                case 2 -> player.addItem(new ItemStack(ModItems.AMULET.get(), 1));
