...
public enum GrindingBowlProvider implements IBlockComponentProvider {
    ...
    public void appendTooltip(
        ...
        Optional<RecipeHolder<GrindingBowlRecipe>> recipe = accessor.getLevel()
                .getRecipeManager()
                .getRecipeFor(RecipeTypes.GRINDING_TYPE.get(), new SingleRecipeInput(input), accessor.getLevel());
        recipe.ifPresent(r -> tooltip.add(Component.translatable(
                "jade.circleofcraft.grinding_bowl.output", r.value().getResult().getHoverName())));
