...
/**
 * Tunnah Diggah — AoE mining tool that breaks stone, dirt, and grass in a cube radius. Biggah
 * Diggah enchantment increases the radius. Precision enchantment increases drop rate.
 */
public class TunnahDiggahItem extends PickaxeItem {
    ...
    private static boolean isAoETarget(BlockState state) {
        return state.is(net.minecraft.world.level.block.Blocks.DIRT)
                ...
                || state.is(ModBlocks.PRIDESTONE.get())
                || state.is(ModBlocks.CORRUPT_PRIDESTONE.get());
    }
