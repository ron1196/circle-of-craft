...
/**
 * A read-only trade slot in Timon's shop. Picking up an item costs {@code bugCost} bugs from the
 * player's inventory. The slot immediately restocks after purchase (infinite supply).
 */
public class TimonTradeSlot extends SlotItemHandler {
    ...
    private int countBugsInInventory(Player player) {
        ...
        for (int i = 0; i < player.getInventory().getContainerSize(); i++) {
            ...
            if (stack.is(ModItems.BUG.get())) count += stack.getCount();
        }
        return count;
    }
